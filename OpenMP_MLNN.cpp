...
const auto NUMBER_OF_HIDDEN = 100;
const auto EXAMPLES_PERCENT = 1;
const auto MAX_ACCURACY = 100;
...
double reportAcc(const MatrixXd & data, const MatrixXd & gt, MatrixXd & i2h, MatrixXd & h2o) {
    ...
    for(int i = 0; i < iterations; ++i) {
        ...
        MatrixXd hidden(i2h.rows(), 1);
        MatrixXd output(h2o.rows(), 1);

        #pragma omp parallel for
        for(int l = 0; l < i2h.rows(); ++l) {
            double sum = 0;
            for(int k = 0; k < i2h.cols(); ++k)
                sum += (i2h.row(l)[k] * data.col(i)[k]);
            hidden(l,0) = 1/(1+exp(-sum));
        }

        #pragma omp parallel for
        for(int l = 0; l < h2o.rows(); ++l) {
            double sum = 0;
            for(int k = 0; k < h2o.cols(); ++k)
                sum += (h2o.row(l)[k] * hidden.col(0)[k]);
            output(l,0) = 1/(1+exp(-sum));
        }
...
double epoch(const MatrixXd & data, const MatrixXd & gt, MatrixXd & i2h, MatrixXd & h2o) {
    ...
    for(int i = 0; i < iterations; ++i) {
        ...
        MatrixXd errOut(output.rows(), 1);
        MatrixXd errHid(hidden.rows(), 1);

        #pragma omp parallel for
        for(int l = 0; l < errOut.rows(); ++l) {
            double oVal = output.col(0)[l];
            double gtVal = gt.col(i)[l];
            errOut(l,0) = (oVal - (oVal*oVal)) * (gtVal - oVal);
        }

        MatrixXd t = h2o.transpose();
        #pragma omp parallel for
        for(int l = 0; l < errHid.rows(); ++l) {
            double sum = 0;
            for(int k = 0; k < t.cols(); ++k)
                sum += (t.row(l)[k] * errOut.col(0)[k]);

            double hVal = hidden.col(0)[l];
            errHid(l,0) = (hVal - (hVal*hVal))*(sum);
        }
        ...
        MatrixXd h2oDelta(h2o.rows(), h2o.cols());
        MatrixXd i2hDelta(i2h.rows(), i2h.cols());
        MatrixXd ht = hidden.transpose();

        #pragma omp parallel for
        for(int l = 0; l < errOut.rows(); ++l) {
            double temp = errOut.col(0)[l] * LEARNING_RATE;
            for(int k = 0; k < ht.cols(); ++k)
                h2oDelta(l,k) = temp * ht.row(0)[k];
        }

        MatrixXd dct = data.col(i).transpose();
        #pragma omp parallel for
        for(int l = 0; l < errHid.rows(); ++l) {
            double temp = errHid.col(0)[l] * LEARNING_RATE;
            for(int k = 0; k < dct.cols(); ++k)
                i2hDelta(l,k) = temp * dct.row(0)[k];
        }
